// bdecimal.cpp: define el punto de entrada de la aplicación de consola.
//
...
static char *memspn(char *pstr, char chr, unsigned cch)
{
    for ( ; cch > 0 && *pstr == chr; cch--, pstr++);
    return pstr;
}

static char   rgch[40]; // 4 groups of 9, include room for sign
...
STDAPI DecToString(LPDECIMAL pdecIn, ULONG dwFlags, char **pbstrOut)
{
    ...
    SPLIT64   sdlHi;
    SPLIT64   sdlTmp;
    char  *pchDig = &rgch[36+1];  // we back up one before we use it
    char  *pchEnd = &rgch[36];
    char  *pchDP;
    int    cch;
    int       i;
    ...
    if (pchDP < pchDig) {
      ...
      memset(pchDP, '0', (size_t) (pchDig - pchDP));
      pchDig = pchDP;
    }
    else
	{
    ...
    // If there are any decimal places, get decimal separator and leading zero flag.
    cch = 1;

    if (pchDP <= pchEnd) 
	{
		memmove(pchDP + 1, pchDP, (pchEnd - pchDP + 1));
		*pchDP = '.';
		pchEnd++;
		if(pchDP == pchDig)
			*--pchDig = '0';
...
STDAPI Multiply(LPDECIMAL pdecL, LPDECIMAL pdecR, LPDECIMAL pdecRes)
{
    SPLIT64 sdlTmp;
    SPLIT64 sdlTmp2;
    SPLIT64 sdlTmp3;
...
STDAPI Divide(LPDECIMAL pdecL, LPDECIMAL pdecR, LPDECIMAL pdecRes)
{
    ULONG   rgulQuo[3];
    ULONG   rgulQuoSave[3];
    ULONG   rgulRem[4];
...
STDAPI Fix(LPDECIMAL pdecOprd, LPDECIMAL pdecRes)
{
    DecFixInt(pdecRes, pdecOprd);
    return NOERROR;
}

STDAPI Int(LPDECIMAL pdecOprd, LPDECIMAL pdecRes)
{
...
STDAPI NativeRound(LPDECIMAL pdecIn, int cDecimals, LPDECIMAL pdecRes)
{
    ULONG   rgulNum[3];
    ULONG   ulRem;
    ULONG   ulSticky;
...
STDAPI DecFromR8(double dblIn, DECIMAL *pdecOut)
{
...
STDAPI VarI4FromR8(double dblIn, int *plOut)
{
