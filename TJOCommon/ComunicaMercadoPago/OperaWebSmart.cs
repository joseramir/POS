...
namespace Alberdi.TJOCommon.ComunicaMercadoPago
{
    public class OperaWebSmart
    {
        ...
        public RtaOrdenCaja CrearOrdenSmartPoint(OrdenCaja peticion)
        {
        ...
        public RtaOrdenCaja ConsultarStatusOrden(string external_reference)
        {
            ...
            string urlWebhook = "http://" + ConfigurationManager.AppSettings["ipsmartpointmp"] + ":" + ConfigurationManager.AppSettings["portsmartpointmp"] +
                "/api/alberdimp/ConsuCaja?external_reference=" + external_reference;
            try
            {
                CantIntentos++;
        ...
        public RtaOrdenCaja ConsultarStatusOrdenMercadoPago(string external_reference)
        {
            ...
            this.ConError = false;
            string urlWebhook = "http://" + ConfigurationManager.AppSettings["ipsmartpointmp"] + ":" + ConfigurationManager.AppSettings["portsmartpointmp"] +
                "/api/alberdimp/ConsuMP?external_reference=" + external_reference;
            try
            {
                CantIntentos++;
                RestClient cliente = new RestClient(urlWebhook);
        ...
        //Para realizar una devolucion de Orden
        public RtaOrdenCaja DevolverOrden(string external_reference)
        {
            RtaOrdenCaja RespuestaDevolucion = null;
            this.ConError = false;
            string urlWebhook = "http://" + ConfigurationManager.AppSettings["ipsmartpointmp"] + ":" + ConfigurationManager.AppSettings["portsmartpointmp"] +
                "/api/alberdimp/DevolverOrden?external_reference=" + external_reference;            
            try
            {
                RestClient cliente = new RestClient(urlWebhook);
                RestRequest reqWebhok = new RestRequest();
                ...
                if (rtaWhebhook.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    RespuestaDevolucion = Newtonsoft.Json.JsonConvert.DeserializeObject<RtaOrdenCaja>(rtaWhebhook.Content.ToString());
                }
                else
                    RespuestaDevolucion = null;
            }
            catch (Exception ex)
            {
                this.ConError = true;
        ...
        {
            StreamWriter writer = File.AppendText(Directory.GetCurrentDirectory() + "\\LogSmartPoint.txt");
