...
{
    /// <summary>
    /// Clase que implementa el algoritmo CRC de 16 bits (suma de comprobacion)
    /// </summary>
    public class CRC16
    {
        /// <summary>
        /// El crc actualmente calculado.
        /// </summary>
        int crc;

        /// <summary>
        /// Permite conocer el resultado de la operacion, o cambiar el crc a un nuevo valor.
        /// </summary>
        public int Crc 
        { 
            get { return crc; }
            set
            {
                if (value < 0) throw new ArgumentException("El CRC no puede ser negativo");
                crc = value;
            }
        }
 
        /// <summary>
        /// Inicializa la suma de comprobacion.
        /// </summary>
        public CRC16()
        {
            crc = 0;
        ...
        /// <summary>
        /// Agrega a la suma de comprobacion una secuencia de bytes.
        /// </summary>
        /// <param name="data">Los bytes que se van a sumar y agregar a la suma actual.</param>
        /// <param name="i">la primera posicion del array 'data' que se debe procesar.</param>
        /// <param name="j">La cantidad de bytes en 'data' que se deben procesar.</param>
        public void update(byte[] data, int i, int j)
        {
            for (int k = 0; k < j; k++)
                crc = crc >> 8 ^ crc16tab[(crc ^ data[k + i]) & 0xff];
        }

        /// <summary>
        /// Tabla interna con los diferentes valores 'magicos' a usar en el algoritmo CRC 16
        /// </summary>
        int[] crc16tab = new int[] {
