...
{
    public partial class VentaTouchKbd : UserControl
    {
        ...
        public VentaTouchKbd()
        {
            ...
            KbdLogger.Log("Inicializando 'VentaTouchKbd'");
            if (File.Exists("pos\\TouchKeyboard2.def"))
            {
                foreach (string line in File.ReadAllLines("pos\\TouchKeyboard2.def"))
                {
                    string l = line.Trim();
        ...
        private void VentaTouchKbd_Load(object sender, EventArgs e)
        {
            if (this.Colapsable)
            {
